...
class Controller {
    ...
    // returns the controller's watchlist
    FileRepository* get_watchlist() {
        return this->watchlist;
    }

    // sets the controller's watchlist to a given FileRepository
     ...
     * throws: invalid_argument if the arguments are invalid or if a Movie with the same id already
     * exists in the repo
     */
    void database_add(const string& title, const string& genre, int year, int likes,
    ...
    /* creates a new Movie from the given parameters and updates the Movie with the given id with
     * the newly created Movie input: id: the id of the Movie from the database to be updated the
     * rest of the parameters: used to create a new Movie throws: invalid_argument if the Movie is
     * invalid or out_of_range if tbe Movie with the given id does not exist
     */
    void database_update(const string& id, const string& title, const string& genre, int year,
                         int likes, const string& trailer);
