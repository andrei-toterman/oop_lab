...
class DynamicVector {
     ...
     */
    explicit DynamicVector(int _cap = 3);
    /*
     * copy constructor
...
DynamicVector<T>::DynamicVector(const DynamicVector<T>& other) {
    ...
    this->len   = 0;
    this->elems = new T[this->cap];
    for (T elem : other) this->elems[this->len++] = elem;
}
...
T& DynamicVector<T>::operator[](int index) const {
    if (index < 0 || index >= this->len) throw std::out_of_range("invalid index");
...
void DynamicVector<T>::remove(int index) {
    if (index < 0 || index >= this->len) throw std::out_of_range("invalid index");
    for (int i = index; i < this->len - 1; ++i) this->elems[i] = this->elems[i + 1];
    this->len--;
}
...
void DynamicVector<T>::resize() {
    ...
    for (int i = 0; i < this->len; ++i) new_elems[i] = this->elems[i];
    delete[] this->elems;
